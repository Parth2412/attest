...
`https://github.com/Parth2412/attest-action-proof/actions/runs/35820332508` rejected
`action/action.yml` at line 36 because `${{ github.token }}` was used in `runs.env`; the parser
reported an unrecognized `github` named value. The required `attest` check was created by GitHub
Actions App ID `15368`, and the protected pull request remained blocked, so the failure was visible
and fail-closed. GitHub documents the `github.token` property as available within execution steps,
while Docker Action metadata documents `runs.args` expressions through the `inputs` context and
does not define `runs.env` as a token injection boundary.

**Decision.** `action/action.yml` contains no `github` context reference and retains only its
closed public inputs, outputs, immutable image digest, and input-based arguments. The exact F-10
generated workflow passes `${{ github.token }}` to the attest step as the `GITHUB_TOKEN` step
environment variable. This is an ambient caller-workflow handoff, not a public Action input,
repository secret, output, command-line argument, or logged value.

The correction is released as `attest-cli==0.1.1`, with exact dependencies on the unchanged five
`0.1.0` libraries, and as immutable Action `v1.0.1`. A source record `v0.1.1` identifies the
reviewed correction commit, and the reviewed moving-major `v1` advances only after public proof.
The five unchanged Python distributions, `ghcr.io/parth2412/attest:0.1.0`, and its immutable
manifest digest are not rebuilt or republished. `v1.0.0` remains immutable and is documented as
defective rather than moved or deleted. The abandoned proof pull request remains evidence and is
closed only after a replacement pull request generated by CLI `0.1.1` exercises Action `v1.0.1`.

The patch publication uses the existing `pypi-attest-cli` Trusted Publisher environment, builds
the CLI wheel and source archive once from the reviewed commit, validates metadata and exact
internal pins, retains hashes and publication evidence, and verifies the public files byte for
...
state exactly which public artifacts changed and avoid rebuilding the already reviewed runtime
image for a metadata/generator-only defect.

**Rejected alternatives.** A public token input expands the credential surface and invites unsafe
forwarding. A repository secret contradicts zero-secret onboarding. Passing the token as an Action
argument exposes it through process metadata. Moving or deleting `v1.0.0` would falsify an
immutable public record. Republishing all six Python distributions or rebuilding the container
would create unrelated bytes and authority without correcting the metadata boundary. Reusing
version `0.1.0` or tag `v1.0.0` is impossible on immutable registries and would obscure the defect.

**Consequences.** Existing `v1.0.0` consumers must update to `v1.0.1`, `v1`, or a corrected full
commit SHA. The exact generated workflow gains one two-line `env` mapping. Patch-release evidence
must prove that only the CLI artifacts and Action/source records changed, the five exact library
pins and image digest did not change, and the corrected public proof traverses every required
blocked, denied, approved, malicious-repository, and fork state before F-11 can be Done.
...
## ADR-051 — Bind generated identities to pull-request merge refs

**Status:** Accepted · **Date:** 2026-09-23 · **Affects:** `TECH-001`, `QA-001`, `BRD-F10`,
`BRD-F11`, `F-10`, `F-11` · **Amends:** `ADR-038`, `ADR-046`, `ADR-049`, `ADR-050`

**Context.** The replacement public proof pull request reached the released `v1.0.1` container,
collected its GitHub evidence, signed a Bundle, and published that Bundle to its git-ref store.
Cryptographic verification then failed because the generated configuration required workflow
identity `.../.github/workflows/attest.yml@refs/heads/main`, while the certificate URI SAN was
`.../.github/workflows/attest.yml@refs/pull/2/merge`. GitHub documents that an open
`pull_request` run uses `refs/pull/<number>/merge`; the public run and certificate independently
confirmed that value. The verifier correctly rejected the non-matching identity with
`ERR-VERIFY-013`.

The CLI represented this verification failure as a fail-closed policy denial. The Action wrapper
then attempted to extract verified facts from the failed verification result and replaced the
known diagnostic with `ERR-INTERNAL-001`. This did not admit an untrusted attestation or make the
required check pass, but it obscured the actionable cause.

**Decision.** `attest init` generates one bounded workflow identity for its pull-request-only
workflow:
`https://github.com/<owner>/<repository>/.github/workflows/attest.yml@refs/pull/*/merge`.
The literal repository and workflow path remain exact, `*` matches only one non-slash
pull-request-identifier segment under the closed `ADR-038` grammar, and the exact issuer remains
mandatory.
Verification still resolves exactly one matching certificate URI SAN and passes that resolved
exact identity to Sigstore; zero or multiple matches fail.

For `run` and `gate`, the Action wrapper recognizes only the closed `ERR-VERIFY-*` codes in a
structurally consistent denied report whose verification status is `failed`. It emits the stable
verification diagnostic and its fatal exit before deriving or writing facts. An unknown code,
inconsistent outcome, unexpected verified identity, statement, or transparency result remains an
internal error. No output or summary fact may be emitted from an unverified Statement.

The correction is published as `attest-cli==0.1.2`, immutable Action `v1.0.2`, source record
`v0.1.2`, and a new multi-platform `ghcr.io/parth2412/attest:0.1.2` manifest because the wrapper
bytes changed. CLI `0.1.2` keeps exact `0.1.0` pins to the five unchanged libraries. A successful
protected `dev` candidate build, scan, SBOM, provenance, and artifact attestation must precede the
reviewed digest pin and protected `main` release. The release promotes that exact candidate
manifest without rebuilding, publishes only the CLI Python distribution, and verifies every
public byte and identity before creating immutable records.

Existing `v0.1.0`, `v0.1.1`, `v1.0.0`, `v1.0.1`, PyPI files, and image manifests remain unchanged.
The moving `v1` tag remains on the first release until a replacement public proof generated by CLI
`0.1.2` exercises Action `v1.0.2` through the blocked, denied, approved, malicious-content, and
fork states. The two earlier failed proof pull requests remain evidence and are closed only after
that replacement succeeds.

**Rationale.** The workflow identity must describe the event that actually requests the workload
certificate. A bounded ref is necessary because the pull-request number does not exist when the
three onboarding files are generated; binding the repository and workflow path literally while
allowing exactly one PR-identifier position preserves the security property. A new image is
unavoidable because correcting the wrapper changes runtime bytes, and immutable release records
must never be repointed.

**Rejected alternatives.** Pinning the default branch repeats the observed mismatch. Accepting any
identity removes the core verification property. Generating a per-pull-request configuration is
not possible at repository initialization and would mutate reviewed policy for every pull request.
Switching to `pull_request_target` would expose privileged execution to untrusted repository
content and remains forbidden. Reusing or moving an immutable version, overwriting the existing
image tag, or rebuilding during release would destroy the reviewed supply-chain boundary. Leaving
the wrapper diagnostic as `ERR-INTERNAL-001` would conceal a closed public verification failure.

**Consequences.** The exact generated config and policy golden bytes change. CLI and Action users
must advance to the new immutable versions for working generated pull-request verification. The
release needs a new reviewed image candidate and public image promotion in addition to the
CLI-only PyPI patch. The five library projects remain untouched. Release completion and the
moving-major tag remain blocked on the full public proof.

The ref behavior was checked against GitHub's
[variables reference](https://docs.github.com/en/actions/reference/workflows-and-actions/variables),
[pull-request event documentation](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows),
and [OIDC claim reference](https://docs.github.com/en/actions/reference/security/oidc), as read on
2026-09-23.
