...
`SCOPE-01`, `SCOPE-04`. Implements `SPEC-001` §3, §4, §5 (algorithm), §11, §12.
...
| `REQ-F01-020` | Python fields **MUST** be `snake_case`; serialisation aliases **MUST** be `camelCase` per `GLOSS-001 §4`. Serialisation **MUST** use aliases by default. |
...
| `REQ-F01-090` | Digest fields **MUST** validate as exactly 64 lowercase hex characters. |
...
| `REQ-F01-120` | `generate_json_schema()` **MUST** produce the structural schema from the models. The schema **MUST NOT** be hand-authored or hand-edited. Cross-field invariants that generated JSON Schema cannot express **MUST** remain enforced by runtime model validators (`ADR-021`). |
...
| `REQ-F01-170` | `AuthorshipMode` **MUST NOT** default to `human-authored`. Absence of information maps to `unknown`. |
...
| `AC-F01-020` | `Statement.model_dump(by_alias=True)` emits only camelCase keys; round-trip through `model_validate` is lossless. |
...
| `AC-F01-170` | `Authorship(claims=[], claims_present=False)` yields `mode == "unknown"` unless explicitly set. |
